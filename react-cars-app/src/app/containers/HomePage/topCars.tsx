...
export function TopCars() {
  ...
  const topChoice = topCars.filter(e => e.dailyPrice < 100);
  const sortedTopChoice = topChoice.sort((first, second) => 0 - (first.dailyPrice > second.dailyPrice ? -1 : 1));
  
  const cars =
    (!isEmptyTopCars &&
      sortedTopChoice.map((car) => <Car  {...car} thumbnailSrc={car.thumbnailUrl} />)) ||
    [];
